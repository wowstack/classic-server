...
enum LogFilters
{
    ...
    LOG_FILTER_DB_STRICTED_CHECK =
        0x002000,                       // 13 stricted DB data checks output (with possible false reports) for DB devs
...
class Log : public MaNGOS::Singleton<Log, MaNGOS::ClassLevelLockable<Log, std::mutex>>
{
  ...
  public:
    ...
    void outWorldPacketDump(const char *socket, uint32 opcode, char const *opcodeName, ByteBuffer const &packet,
                            bool incoming);
    // any log level
