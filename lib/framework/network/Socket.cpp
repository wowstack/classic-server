...
namespace MaNGOS
{
    ...
    void Socket::StartAsyncRead()
    {
        ...
        m_socket.async_read_some(
            boost::asio::buffer(&m_inBuffer->m_buffer[m_inBuffer->m_writePosition],
                                m_inBuffer->m_buffer.size() - m_inBuffer->m_writePosition),
            make_custom_alloc_handler(m_allocator, [ptr](const boost::system::error_code &error, size_t length) {
                ptr->OnRead(error, length);
            }));
    }
    ...
    void Socket::OnRead(const boost::system::error_code &error, size_t length)
    {
        ...
        while (m_inBuffer->m_readPosition < m_inBuffer->m_writePosition)
        {
            ...
            {
                // this errno is set when there is not enough buffer data available to either complete a header, or the
                // packet length specified in the header goes past what we've read.  in this case, we will reset the
                // buffer with the remaining data
                if (errno == EBADMSG)
                {
                    const size_t bytesRemaining = m_inBuffer->m_writePosition - m_inBuffer->m_readPosition;

                    ::memmove(&m_inBuffer->m_buffer[0], &m_inBuffer->m_buffer[m_inBuffer->m_readPosition],
                              bytesRemaining);
    ...
    void Socket::FlushOut()
    {
        ...
        m_socket.async_write_some(
            boost::asio::buffer(m_outBuffer->m_buffer, m_outBuffer->m_writePosition),
            make_custom_alloc_handler(m_allocator, [ptr](const boost::system::error_code &error, size_t length) {
                ptr->OnWriteComplete(error, length);
            }));
    }
    ...
    void Socket::OnWriteComplete(const boost::system::error_code &error, size_t length)
    {
        ...
        if (m_secondaryOutBuffer->m_writePosition > 0)
        {
            ...
            memcpy(&(m_outBuffer->m_buffer[m_outBuffer->m_writePosition]), &(m_secondaryOutBuffer->m_buffer[0]),
