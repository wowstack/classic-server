...
enum FieldFormat
{
    FT_NA = 'x', // ignore/ default, 4 byte size, in Source String means field is ignored, in Dest String means field is
    ...
    FT_NA_POINTER =
        'p',          // fill default value into dest, pointer size, Use this only with static data (otherwise mem-leak)
    FT_STRING = 's',  // char*
    FT_FLOAT = 'f',   // float
    FT_INT = 'i',     // uint32
    FT_BYTE = 'b',    // uint8
...
class DBCFileLoader
{
  public:
    ...
    {
        return (fieldsOffset != nullptr && id < fieldCount) ? fieldsOffset[id] : 0;
    }
    bool IsLoaded() const
    {
        return data != nullptr;
